...
RUN apt-get update \
   # Core PHP modules \
   && apt install -y --no-install-recommends libicu-dev libxml2-dev libbz2-dev libmcrypt-dev zlib1g-dev libc-client-dev libkrb5-dev libmagickwand-dev libxslt-dev mysql-client \
   ...
   && git clone --recursive https://github.com/absalomedia/sassphp.git /tmp/sassphp \
   && cd /tmp/sassphp \
   ...
   \
   # Additional apps
   && apt install -y --no-install-recommends nano wget ghostscript less zip unzip \
   \
   # Cleanup
