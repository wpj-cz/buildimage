...
RUN apt-get update \
   # Core PHP modules \
   && apt install -y --no-install-recommends libonig-dev libicu-dev libxml2-dev libjpeg62-turbo-dev libbz2-dev libzip-dev libmcrypt-dev zlib1g-dev libc-client-dev libkrb5-dev libmagickwand-dev libxslt-dev mariadb-client \
   ...
   && pecl install memcached imagick apcu mcrypt \
   && docker-php-ext-enable memcached imagick apcu mcrypt \
   \
